 ...
 *
 * API Reference (from Azure AI Foundry deployment page):
 *   POST /openai/v1/videos
 *   Header: Api-key: <key>
 *   Body: { model, prompt, size, seconds }
 */
...
class VideoService {
   ...
   * @param {string} options.size - Video resolution (default: 1920x1080)
   * @param {number} options.duration - Duration in seconds (default: 5)
   * @param {number} options.count - Number of videos to generate (default: 1)
   ...
   * @param {Buffer|string} imageSource - Image buffer (base64 encoded) or URL
   * @param {string} prompt - Video generation prompt
   * @param {Object} options - Generation options
   * @returns {Promise<string>} Job ID for polling status
   * @throws {Error} If API request fails
  ...
  async generateFromImage(imageSource, prompt, options = {}) {
    try {
      ...
      console.log(
        `[VideoService] Starting image-to-video generation: "${prompt.substring(0, 50)}..."`
      );
      ...
      if (Buffer.isBuffer(imageSource)) {
        ...
        imageData = { base64_image: `data:image/png;base64,${base64}` };
      } else {
  ...
  async checkStatus(jobId) {
    try {
      ...
      // Normalize status — Sora 2 may return various status strings
      ...
      console.error('[VideoService] Status check failed:', error.response?.status, error.response?.data || error.message);
      throw new Error(
  ...
  async downloadVideo(jobId) {
    try {
      ...
      console.error('[VideoService] Video download failed:', error.response?.status, error.message);
      throw new Error(
  ...
  async validateConnection() {
    try {
      // Just try a simple request to verify credentials work
      // We'll create and immediately check — if auth works, we're good
