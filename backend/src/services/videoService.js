 ...
 *   Header: Api-key: <key>
 *   
 *   Text-to-video body: { model, input: [{ type: "input_text", text: "..." }], size, seconds }
 *   Image-to-video body: { model, input: [{ type: "input_image", image_base64: "..." }, { type: "input_text", text: "..." }], size, seconds }
 *
...
class VideoService {
  ...
  /**
     * Generates a video from text prompt using Sora 2 input format
  ...
  /**
     * Generates a video from an image and optional text prompt using Sora 2 input format
     * Uses the "input" array with typed objects as per the Sora 2 API spec
  ...
  async generateFromImage(imageSource, prompt, options = {}) {
        try {
          ...
          const promptText = (prompt && typeof prompt === 'string') ? prompt.substring(0, 50) : '(no prompt)';
                console.log(
          ...
          console.log(`[VideoService] Image-to-video payload (image truncated):`, JSON.stringify({
                    ...payload,
                    input: payload.input.map(i => i.type === 'input_image' ? { ...i, image_base64: i.image_base64 ? `[${i.image_base64.length} chars]` : undefined } : i),
          }, null, 2));
      ...
      while (attempt < maxAttempts) {
              try {
                ...
                if (status.status === 'failed') {
                            const errorMsg =
                                          status.error?.message || 'Video generation failed without error details';
