...
class ApiClient {
  ...
  async listVideos(): Promise<VideoHistoryItem[]> {
    ...
    const result = await this.handleResponse<{ success: boolean; data: { videos: VideoHistoryItem[] } }>(response);
...
export const generateVideoFromImage = (
  ...
  duration?: number,
  resolution?: string
) => apiClient.generateVideoFromImage(file, prompt, duration, resolution);
